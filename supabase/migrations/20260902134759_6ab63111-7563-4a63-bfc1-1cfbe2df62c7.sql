-- Repatriation migration: these three functions existed in the live database
-- without any migration file. Definitions below are byte-for-byte what
-- pg_get_functiondef() returns today. NO logic change.

CREATE OR REPLACE FUNCTION public.admin_review_car_pass(_listing_id uuid, _decision text, _note text DEFAULT NULL::text)
 RETURNS TABLE(out_listing_id uuid, out_car_pass_status text)
 LANGUAGE plpgsql
 SECURITY DEFINER
...
CREATE OR REPLACE FUNCTION public.get_public_seller_identity(_user_id uuid)
 RETURNS TABLE(user_id uuid, display_name text, garage_name text, avatar_url text, user_type text, created_at timestamp with time zone, is_admin boolean)
 LANGUAGE sql

...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    const {
      ...
      year,
      status,
    }: NotifyListingStatusRequest = await req.json();

    console.log("Listing status notification:", { sellerEmail, brand, model, status });
    ...
    const approvedMessage = `
      ...
      <p>Elle est désormais <strong>visible par tous les acheteurs</strong> sur AutoRa. Vous pouvez suivre ses performances depuis votre tableau de bord.</p>
    `;
          ...
          <body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px; background: #f0f0f0;">
            ...
            <div style="background: #ffffff; padding: 30px; border-radius: 0 0 12px 12px;">
              <div style="text-align: center; margin-bottom: 24px;">
                <span style="display: inline-block; background: ${statusColor}15; color: ${statusColor}; padding: 8px 20px; border-radius: 20px; font-weight: 600; font-size: 16px;">
                  ${statusIcon} Annonce ${statusText}
                </span>
              </div>
              ...
              <div style="text-align: center; margin-top: 28px;">
                <a href="${appUrl}/dashboard" 
                   style="display: inline-block; background: #3b82f6; color: #fff; padding: 12px 32px; text-decoration: none; border-radius: 8px; font-weight: 500;">
              ...
              <p style="color: #94a3b8; font-size: 12px; margin-bottom: 0; text-align: center;">
                ...
                Vous recevez cet email car vous avez soumis une annonce sur notre plateforme.
              </p>
            </div>
          </body>
        </html>

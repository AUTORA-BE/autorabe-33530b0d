...
export default defineConfig(({ mode }) => {
  // Vite does NOT populate process.env with VITE_* variables — loadEnv reads
  // the .env files (plus real process.env) so build-time plugins can use them.
  ...
  return {
    plugins: [
      ...
      prerenderOgMeta({ supabaseUrl, anonKey }),
      mcpPlugin(),
    ],
